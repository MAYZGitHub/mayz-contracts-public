...
import qualified Ledger                         as LedgerApiV2
...
pabUTxOAtScript protocolPABParams' = do
    ...
    case protocolPABParams' of
        (Just protocolPABParams) -> do
            ...
            PABHelpers.showUTxOsAtAddress @InvestUnitT.ValidatorDatum (T.ffppInvestUnitValidator_Address $ head $ T.pppFundFactoryPABParams protocolPABParams)
    ...
    let validatorsNameAndHashes = case protocolPABParams' of
            Just protocolPABParams ->
                    ...
                    in  [
                            ...
                            ("Invest Units", T.ffppInvestUnitValidator_Hash $ head $ T.pppFundFactoryPABParams protocolPABParams),
...
isCoreTeam (Just walletNro) = LedgerApiV2.unPaymentPubKeyHash (PABHelpers.walletPaymentPubKeyHash walletNro) `P.elem` T.coreTeamWallets
...
isProtocolAdmin (Just walletNro) (Just protocolPABParams) = do
        ...
        let !uTxOuts = PABHelpers.getUTxOsListInPABSimulator blockchain (T.pppProtocolValidator_Address protocolPABParams)
            !uTxO_With_ProtocolDatum' = [ (txOutRef, txOut,  ProtocolT.getProtocolDatumType $ OnChainHelpers.fromJust $ PABHelpers.getDatumInPABSimulator @ProtocolT.ValidatorDatum blockchain (txOutRef, txOut) )
        ...
        ---------------------
        return $ LedgerApiV2.unPaymentPubKeyHash (PABHelpers.walletPaymentPubKeyHash walletNro) `P.elem` protocolAdmins
...
isMAYZHolderAdmin (Just walletNro) (Just protocolPABParams) = do
        ...
        protocolAdmins <- case uTxO_With_ProtocolDatum' of
            ...
            (uTxO_With_ProtocolDatum:_) -> do
                ---------------------
                let !protocolDatum = (\(_, _, dat) -> dat) uTxO_With_ProtocolDatum
                return $ ProtocolT.pdMAYZWallets protocolDatum
        ---------------------
        return $ LedgerApiV2.unPaymentPubKeyHash (PABHelpers.walletPaymentPubKeyHash walletNro) `P.elem` protocolAdmins
...
isMAYZHolder (Just walletNro) = do
...
isFundAdmin (Just walletNro) (Just protocolPABParams) = do
        ...
        let getAdmins :: T.FundPABParams -> MonadFreerInternal.Eff PABContracts.PABEffects [T.WalletPaymentPKH]
            getAdmins fundPABParams = do
                ...
                let !uTxOuts = PABHelpers.getUTxOsListInPABSimulator blockchain (T.fppFundValidator_Address fundPABParams)
                    !uTxO_With_FundDatum' = [ (txOutRef, txOut,  FundT.getFundDatumType $ OnChainHelpers.fromJust $ PABHelpers.getDatumInPABSimulator @FundT.ValidatorDatum blockchain (txOutRef, txOut) )
                                                            | (txOutRef, txOut) <- uTxOuts,
        ...
        ---------------------
        return $ LedgerApiV2.unPaymentPubKeyHash (PABHelpers.walletPaymentPubKeyHash walletNro) `P.elem`  fundAdmins
...
isThisFundAdmin (Just walletNro) (Just protocolPABParams) (Just fundPABParams) = do
        blockchain <- PABSimulator.blockchain
        ---------------------
...
pabShowInvestUnit _ (_, _) protocolPABParams fundPABParams' _ _ = do
    ...
    case fundPABParams' of
        (Just fundPABParams) -> do
            ...
            ---------------------
                !uTxO_With_InvestUnitDatum' = [ (txOutRef, txOut,  InvestUnitT.getInvestUnitDatumType $ OnChainHelpers.fromJust $ PABHelpers.getDatumInPABSimulator @InvestUnitT.ValidatorDatum blockchain (txOutRef, txOut) )
                                                        | (txOutRef, txOut) <- uTxOuts,
